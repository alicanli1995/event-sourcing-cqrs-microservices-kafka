...
public class DepositFundsController {
    ...
    public ResponseEntity<BaseResponse> depositFunds(@PathVariable("id") String id,
        ...
        try {
            ...
            return new ResponseEntity<>(new BaseResponse("Bank account deposit successfully"), HttpStatus.OK);
        }catch (IllegalStateException e){
            log.warn("Client made a bad request -> " + e);
            return new ResponseEntity<>(new BaseResponse(e.toString()), HttpStatus.BAD_REQUEST);
        }catch (Exception e){
            var errorMsg = MessageFormat.format("Error while processing request to deposit bank account for id -> {0}" , id);
